...
$messageWarning = "votre requete a été envoyé";
...
function Annoncesdernierpublier(PDO $pdo) : array
{
    $req = $pdo->query(
        'SELECT *
        FROM membre
        ORDER BY date_enregistrement DESC LIMIT 0, 10'
    );
...
<img  class="mt-4" src="assets/img/radio.png" alt="radiorasta" style="width:100%;">
          </div>
      </div>
  </div>
...
if ($post['civilite'] == "femme") {
   ...
   ?>
   <span id="countGifts" style="background-color:rgb(255, 0, 170);font-size:14px;color:white;border:2px solid;border-radius:50%;padding:5px;position:relative;float:right;z-index:1;">  <?php echo $datas->totalcadeaux; ?> </span>
   <?php 
}
else {
    echo '<img src="assets/img/lion.png" alt"weed" style="width:20%;height:20%;position:relative;float:right;z-index:1;"/>';
    ?>
       <span id="countGifts" style="background-color:dodgerblue;font-size:14px;color:white;border:2px solid;border-radius:50%;padding:5px;position:relative;float:right;z-index:1;">  <?php echo $datas->totalcadeaux; ?> </span>
    <?php 
}
...
<div class="card-body">
<h4 class="card-title" style="color:white;"><?= nl2br(htmlspecialchars($post['pseudo'])) ?></h4><?php if($post['connexion'] == 1):?>
...
<p class="card-text" style="color:white;"><?= nl2br(htmlspecialchars(substr($post['description_membre'],0,20) . '...')); ?></p>
                     <?php else: ?>
 <p class="card-text" style="color:white;"> <?= $post['description_membre']; ?></p>
                        <?php endif; ?>
</div>
</div>
</div>
...
<?php endwhile;
  ...
  ?>
    ...
    <div class="col-12 col-md-5">
            ...
            <form action="accueil.php" method="post" class="form-group">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLongTitle">Vous avez des Messages</h5>
...
$NewMessage = $allNewMessage ->fetch();
...
<?php  var_dump( $NewMessage['id_message']);  ?>
<?php  var_dump( $NewMessage['membre_id1']);  ?>

<form action=" " method="post">

<input  class="btn btn-primary" type="submit" value="ajout" name="addingfr">
</form>
<!--<div   class= "btn btn-primary"  id="addfriendz">ajouter</div>
...
?>
 ...
 <?php while($NewMessage = $allNewMessage ->fetchAll()) :?>
      </div>
